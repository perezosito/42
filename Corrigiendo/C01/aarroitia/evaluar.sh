...
extract_main_and_libraries() {
  ...
  # Elimina las líneas de comentarios dentro del bloque
  echo "$output" | grep -v '^\s*\*'
}
...
for file in $files; do
  ...
  # Añade main y librerías al final del archivo combinado
  ...
  gcc -Wall -Wextra -Werror "$combined_file" -o "${combined_file%.c}.out" -I. -include unistd.h
...
gedit $(find . -name "ex*" -type f -name "*.c")
